...
status_t Surface::getWideColorSupport(bool* supported) {
    ...
    sp<IBinder> display(composerService()->getBuiltInDisplay(ISurfaceComposer::eDisplayIdMain));
    Vector<android_color_mode_t> colorModes;
    status_t err = composerService()->getDisplayColorModes(display, &colorModes);
...
status_t Surface::getHdrSupport(bool* supported) {
    ...
    status_t err = composerService()->getHdrCapabilities(display, &hdrCapabilities);
...
int Surface::queueBuffer(android_native_buffer_t* buffer, int fenceFd) {
    ...
    IGraphicBufferProducer::QueueBufferInput input(timestamp, isAutoTimestamp, mDataSpace, crop,
...
void Surface::querySupportedTimestampsLocked() const {
    ...
    status_t err = composerService()->getSupportedFrameTimestamps(&supportedFrameTimestamps);
    ...
    { // scope for the lock
        ...
        switch (what) {
            ...
            case NATIVE_WINDOW_QUEUES_TO_WINDOW_COMPOSER: {
                if (composerService()->authenticateSurfaceTexture(mGraphicBufferProducer)) {
                    *value = 1;
                } else {
                    *value = 0;
                }
...
int Surface::dispatchSetBuffersDataSpace(va_list args) {
    android_dataspace dataspace = static_cast<android_dataspace>(va_arg(args, int));
...
int Surface::detachNextBuffer(sp<GraphicBuffer>* outBuffer, sp<Fence>* outFence) {
    ...
    *outBuffer = buffer;
    ...
    for (int i = 0; i < NUM_BUFFER_SLOTS; i++) {
        if (mSlots[i].buffer != NULL && mSlots[i].buffer->getId() == buffer->getId()) {
            if (mReportRemovedBuffers) {
...
status_t Surface::lock(ANativeWindow_Buffer* outBuffer, ARect* inOutDirtyBounds) {
    ...
    if (err == NO_ERROR) {
        ...
        const bool canCopyBack = (frontBuffer != 0 && backBuffer->width == frontBuffer->width &&
...
status_t Surface::lockWithHandle(ANativeWindow_Buffer* outBuffer, buffer_handle_t* handle,
