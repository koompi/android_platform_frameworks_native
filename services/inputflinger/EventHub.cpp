...
EventHub::EventHub(void)
    ...
    LOG_ALWAYS_FATAL_IF(result < 0, "Could not register INotify for %s.  errno=%d", DEVICE_PATH,
                        errno);
        ...
        if (device && device->hasValidFd() && test_bit(axis, device->absBitmask)) {
            ...
            if (ioctl(device->fd, EVIOCGABS(axis), &info)) {
                ALOGW("Error reading absolute controller %d for device %s fd %d, errno=%d", axis,
                      device->identifier.name.string(), device->fd, errno);
                return -errno;
            }
...
void EventHub::configureFd(Device* device) {
    ...
    if (device->classes & INPUT_DEVICE_CLASS_KEYBOARD) {
        ...
        if (ioctl(device->fd, EVIOCSREP, repeatRate)) {
            ALOGW("Unable to disable kernel key repeat for %s: %s", device->path.string(),
                  strerror(errno));
        }
    }
...
void EventHub::closeDeviceLocked(Device* device) {
    ALOGI("Removed device: path=%s name=%s id=%d fd=%d classes=0x%x\n", device->path.string(),
          device->identifier.name.string(), device->id, device->fd, device->classes);
