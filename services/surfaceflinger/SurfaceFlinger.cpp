...
#define PROPERTY_PIONUXOS_DESKTOP_INTERACTIVE "sys.pionuxos.desktop.interactive"
...
SurfaceFlinger::SurfaceFlinger()
    ...
    vsyncPhaseOffsetNs =
            getInt64<ISurfaceFlingerConfigs, &ISurfaceFlingerConfigs::vsyncEventPhaseOffsetNs>(
                    1000000);

    sfVsyncPhaseOffsetNs =
            getInt64<ISurfaceFlingerConfigs, &ISurfaceFlingerConfigs::vsyncSfEventPhaseOffsetNs>(
                    1000000);
    ...
    useContextPriority =
            getBool<ISurfaceFlingerConfigs, &ISurfaceFlingerConfigs::useContextPriority>(false);

    dispSyncPresentTimeOffset =
            getInt64<ISurfaceFlingerConfigs, &ISurfaceFlingerConfigs::presentTimeOffsetFromVSyncNs>(
                    0);

    useHwcForRgbToYuv =
            getBool<ISurfaceFlingerConfigs, &ISurfaceFlingerConfigs::useHwcForRGBtoYUV>(false);

    maxVirtualDisplaySize =
            getUInt64<ISurfaceFlingerConfigs, &ISurfaceFlingerConfigs::maxVirtualDisplaySize>(0);
...
void SurfaceFlinger::createBuiltinDisplayLocked(DisplayDevice::DisplayType type) {
    ...
    ALOGW_IF(mBuiltinDisplays[type], "Overwriting display token for display type %d", type);
...
void SurfaceFlinger::bootFinished() {
    ...
    sp<LambdaMessage> readProperties = new LambdaMessage([&]() { readPersistentProperties(); });
...
void SurfaceFlinger::init() {
    ...
    sp<VSyncSource> vsyncSrc =
            new DispSyncSource(&mPrimaryDispSync, vsyncPhaseOffsetNs, true, "app");
    mEventThread = new EventThread(vsyncSrc, *this, false);
    sp<VSyncSource> sfVsyncSrc =
            new DispSyncSource(&mPrimaryDispSync, sfVsyncPhaseOffsetNs, true, "sf");
    ...
    mRenderEngine =
            RenderEngine::create(mEGLDisplay, HAL_PIXEL_FORMAT_RGBA_8888,
    ...
    if (useVrFlinger) {
        ...
        mVrFlinger = dvr::VrFlinger::Create(mHwc->getComposer(), vrFlingerRequestDisplayCallback);
        if (!mVrFlinger) {
...
status_t SurfaceFlinger::getDisplayConfigs(const sp<IBinder>& display,
    ...
    public:
        static int getEmuDensity() { return getDensityFromProperty("qemu.sf.lcd_density"); }
        static int getBuildDensity() { return getDensityFromProperty("ro.sf.lcd_density"); }
    };
    ...
    for (const auto& hwConfig : getHwComposer().getConfigs(type)) {
        ...
        info.presentationDeadline = hwConfig->getVsyncPeriod() - sfVsyncPhaseOffsetNs + 1000000;
...
status_t SurfaceFlinger::getDisplayStats(const sp<IBinder>& /* display */, DisplayStatInfo* stats) {
    if (stats == NULL) {
        return BAD_VALUE;
    }
...
void SurfaceFlinger::setActiveConfigInternal(const sp<DisplayDevice>& hw, int mode) {
    ALOGD("Set active config mode=%d, type=%d flinger=%p", mode, hw->getDisplayType(), this);
...
status_t SurfaceFlinger::setActiveConfig(const sp<IBinder>& display, int mode) {
    ...
    public:
        MessageSetActiveConfig(SurfaceFlinger& flinger, const sp<IBinder>& disp, int mode)
              : mFlinger(flinger), mDisplay(disp) {
            mMode = mode;
        }
        virtual bool handler() {
            Vector<DisplayInfo> configs;
            ...
            if (mMode < 0 || mMode >= static_cast<int>(configs.size())) {
                ALOGE("Attempt to set active config = %d for display with %zu configs", mMode,
                      configs.size());
                return true;
            }
            sp<DisplayDevice> hw(mFlinger.getDisplayDevice(mDisplay));
            if (hw == NULL) {
                ALOGE("Attempt to set active config = %d for null display %p", mMode,
                      mDisplay.get());
            } else if (hw->getDisplayType() >= DisplayDevice::DISPLAY_VIRTUAL) {
                ALOGW("Attempt to set active config = %d for virtual display", mMode);
            } else {
...
void SurfaceFlinger::onVsyncReceived(int32_t sequenceId, hwc2_display_t displayId,
                                     int64_t timestamp) {
...
void SurfaceFlinger::createDefaultDisplayDevice() {
    ...
    sp<DisplayDevice> hw =
            new DisplayDevice(this, DisplayDevice::DISPLAY_PRIMARY, type, isSecure, token, fbs,
                              producer, mRenderEngine->getEGLConfig(), useWideColorMode);
...
void SurfaceFlinger::onHotplugReceived(int32_t sequenceId, hwc2_display_t display,
    ...
    ALOGV("onHotplugReceived(%d, %" PRIu64 ", %s, %s)", sequenceId, display,
...
void SurfaceFlinger::updateVrFlinger() {
    ...
    int currentDisplayPowerMode =
            getDisplayDeviceLocked(mBuiltinDisplays[DisplayDevice::DISPLAY_PRIMARY])
                    ->getPowerMode();
    ...
    sp<DisplayDevice> hw(getDisplayDeviceLocked(mBuiltinDisplays[DisplayDevice::DISPLAY_PRIMARY]));
...
void SurfaceFlinger::updateCompositorTiming(nsecs_t vsyncPhase, nsecs_t vsyncInterval,
                                            nsecs_t compositeTime,
    ...
    setCompositorTimingSnapped(vsyncPhase, vsyncInterval, compositeToPresentLatency);
}

void SurfaceFlinger::setCompositorTimingSnapped(nsecs_t vsyncPhase, nsecs_t vsyncInterval,
    ...
    int64_t extraVsyncs = (compositeToPresentLatency - idealLatency + bias) / vsyncInterval;
    nsecs_t snappedCompositeToPresentLatency =
            (extraVsyncs > 0) ? idealLatency + (extraVsyncs * vsyncInterval) : idealLatency;
...
void SurfaceFlinger::postComposition(nsecs_t refreshStartTime) {
    ...
    if (mHwc->hasClientComposition(HWC_DISPLAY_PRIMARY)) {
        glCompositionDoneFenceTime = std::make_shared<FenceTime>(hw->getClientTargetAcquireFence());
    ...
    updateCompositorTiming(vsyncPhase, vsyncInterval, refreshStartTime, presentFenceTime);
    ...
    mDrawingState.traverseInZOrder([&](Layer* layer) {
        bool frameLatched = layer->onPostComposition(glCompositionDoneFenceTime, presentFenceTime,
                                                     compositorTiming);
        if (frameLatched) {
...
void SurfaceFlinger::rebuildLayerStacks() {
    ...
    if (CC_UNLIKELY(mVisibleRegionsDirty)) {
        ...
        bool isDesktopInteractive = property_get_bool(PROPERTY_PIONUXOS_DESKTOP_INTERACTIVE, false);
        for (size_t dpy = 0; dpy < mDisplays.size(); dpy++) {
            Region opaqueRegion;
            Region dirtyRegion;
            ...
            if (displayDevice->isDisplayOn()) {
                ...
                mDrawingState.traverseInZOrder([&](Layer* layer) {
                    ...
                    if (shouldShowLayer &&
                        layer->belongsToDisplay(displayDevice->getLayerStack(),
            ...
            displayDevice->undefinedRegion.subtractSelf(tr.transform(opaqueRegion));
...
mat4 SurfaceFlinger::computeSaturationMatrix() const {
    ...
    return mat4(vec4{luminance.r + mSaturation, luminance.r, luminance.r, 0.0f},
...
android_dataspace SurfaceFlinger::bestTargetDataSpace(android_dataspace a,
                                                      android_dataspace b) const {
...
void SurfaceFlinger::setUpHWComposer() {
    ...
    for (size_t dpy = 0; dpy < mDisplays.size(); dpy++) {
        ...
        ALOGV_IF(mDisplays[dpy]->getDisplayType() == DisplayDevice::DISPLAY_VIRTUAL,
                 ...
                 mustRecompose ? "doing" : "skipping", dirty ? "+" : "-", empty ? "+" : "-",
                 wasEmpty ? "+" : "-");
    ...
    for (size_t displayId = 0; displayId < mDisplays.size(); ++displayId) {
        ...
        if (colorMatrix != mPreviousColorMatrix) {
            ...
            ALOGE_IF(result != NO_ERROR,
                     "Failed to set color transform on "
                     "display %zd: %d",
                     displayId, result);
        }
                    ...
                    if (state.isVirtualDisplay()) {
                        ...
                        if (state.surface != NULL) {
                            ...
                            sp<VirtualDisplaySurface> vds =
                                    new VirtualDisplaySurface(*mHwc, hwcId, state.surface,
...
void SurfaceFlinger::doDisplayComposition(const sp<const DisplayDevice>& displayDevice,
...
bool SurfaceFlinger::doComposeSurfaces(const sp<const DisplayDevice>& displayDevice,
    ...
    if (hasClientComposition) {
...
#ifdef USE_HWC2
        mRenderEngine->setWideColor(displayDevice->getWideColorSupport() && !mForceNativeColorMode);
        mRenderEngine->setColorMode(mForceNativeColorMode ? HAL_COLOR_MODE_NATIVE
                                                          : displayDevice->getActiveColorMode());
#endif
        ...
        if (displayDevice->getDisplayType() != DisplayDevice::DISPLAY_PRIMARY) {
            ...
            if (scissor != bounds) {
                ...
                mRenderEngine->setScissor(scissor.left, height - scissor.bottom, scissor.getWidth(),
                                          scissor.getHeight());
            }
        }
    }
    ...
    if (hwcId >= 0) {
        ...
        for (auto& layer : displayDevice->getVisibleLayersSortedByZ()) {
            ...
            ALOGV("  Composition type: %s", to_string(layer->getCompositionType(hwcId)).c_str());
            if (!clip.isEmpty()) {
...
status_t SurfaceFlinger::removeLayer(const sp<Layer>& layer, bool topLevelOnly) {
    ...
    if (index < 0 && mDrawingState.layersSortedByZ.indexOf(layer) < 0) {
        ALOGE("Failed to find layer (%s) in layer parent (%s).", layer->getName().string(),
...
uint32_t SurfaceFlinger::setClientStateLocked(const sp<Client>& client, const layer_state_t& s) {
    ...
    if (layer != 0) {
        ...
        if (what & layer_state_t::eCropChanged) {
            if (layer->setCrop(s.crop, !geometryAppliesWithResize)) flags |= eTraversalNeeded;
        }
...
status_t SurfaceFlinger::createLayer(const String8& name, const sp<Client>& client, uint32_t w,
    ...
    switch (flags & ISurfaceComposerClient::eFXSurfaceMask) {
        case ISurfaceComposerClient::eFXSurfaceNormal:
            result =
                    createNormalLayer(client, uniqueName, w, h, flags, format, handle, gbp, &layer);
            break;
        case ISurfaceComposerClient::eFXSurfaceDim:
            result = createDimLayer(client, uniqueName, w, h, flags, handle, gbp, &layer);
            break;
        default:
            result = BAD_VALUE;
            break;
...
status_t SurfaceFlinger::createNormalLayer(const sp<Client>& client, const String8& name,
...
status_t SurfaceFlinger::createDimLayer(const sp<Client>& client, const String8& name, uint32_t w,
                                        uint32_t h, uint32_t flags, sp<IBinder>* handle,
                                        sp<IGraphicBufferProducer>* gbp, sp<Layer>* outLayer) {
...
status_t SurfaceFlinger::onLayerRemoved(const sp<Client>& client, const sp<IBinder>& handle) {
    ...
    if (l != NULL) {
        ...
        err = removeLayer(l);
        ALOGE_IF(err < 0 && err != NAME_NOT_FOUND, "error removing layer=%p (%s)", l.get(),
                 strerror(-err));
    }
    return err;
}
...
void SurfaceFlinger::setPowerModeInternal(const sp<DisplayDevice>& hw, int mode,
                                          bool stateLockHeld) {
    ALOGD("Set power mode=%d, type=%d flinger=%p", mode, hw->getDisplayType(), this);
    int32_t type = hw->getDisplayType();
    ...
    if (currentMode == HWC_POWER_MODE_OFF) {
        ...
        if (type == DisplayDevice::DISPLAY_PRIMARY && mode != HWC_POWER_MODE_DOZE_SUSPEND) {
...
void SurfaceFlinger::setPowerMode(const sp<IBinder>& display, int mode) {
    ...
    public:
        MessageSetPowerMode(SurfaceFlinger& flinger, const sp<IBinder>& disp, int mode)
              : mFlinger(flinger), mDisplay(disp) {
            mMode = mode;
        }
        virtual bool handler() {
            sp<DisplayDevice> hw(mFlinger.getDisplayDevice(mDisplay));
            if (hw == NULL) {
                ALOGE("Attempt to set power mode = %d for null display %p", mMode, mDisplay.get());
            } else if (hw->getDisplayType() >= DisplayDevice::DISPLAY_VIRTUAL) {
                ALOGW("Attempt to set power mode = %d for virtual display", mMode);
            } else {
                mFlinger.setPowerModeInternal(hw, mMode, /*stateLockHeld*/ false);
            }
            return true;
        }
    };
...
status_t SurfaceFlinger::dump(int fd, const Vector<String16>& args) {
    ...
    if ((uid != AID_SHELL) && !PermissionCache::checkPermission(sDump, pid, uid)) {
        ...
        if (!locked) {
            result.appendFormat("SurfaceFlinger appears to be unresponsive (%s [%d]), "
        ...
        if (numArgs) {
            if ((index < numArgs) && (args[index] == String16("--list"))) {
                index++;
                listLayersLocked(args, index, result);
                dumpAll = false;
            }

            if ((index < numArgs) && (args[index] == String16("--latency"))) {
                index++;
                dumpStatsLocked(args, index, result);
                dumpAll = false;
            }

            if ((index < numArgs) && (args[index] == String16("--latency-clear"))) {
                index++;
                clearStatsLocked(args, index, result);
                dumpAll = false;
            }

            if ((index < numArgs) && (args[index] == String16("--dispsync"))) {
                index++;
                mPrimaryDispSync.dump(result);
                dumpAll = false;
            }

            if ((index < numArgs) && (args[index] == String16("--static-screen"))) {
                index++;
                dumpStaticScreenStats(result);
                dumpAll = false;
            }

            if ((index < numArgs) && (args[index] == String16("--frame-events"))) {
                index++;
...
void SurfaceFlinger::listLayersLocked(const Vector<String16>& /* args */, size_t& /* index */,
    ...
    mCurrentState.traverseInZOrder(
            [&](Layer* layer) { result.appendFormat("%s\n", layer->getName().string()); });
}
...
void SurfaceFlinger::appendSfConfigString(String8& result) const {
    ...
    result.appendFormat(" NUM_FRAMEBUFFER_SURFACE_BUFFERS=%" PRId64, maxFrameBufferAcquiredBuffers);
    result.append("]");
}
...
void SurfaceFlinger::dumpStaticScreenStats(String8& result) const {
    ...
    for (size_t b = 0; b < NUM_BUCKETS - 1; ++b) {
        ...
        float percent = 100.0f * static_cast<float>(mFrameBuckets[b]) / mTotalTime;
        result.appendFormat("  < %zd frames: %.3f s (%.1f%%)\n", b + 1, bucketTimeSec, percent);
    }
    float bucketTimeSec = mFrameBuckets[NUM_BUCKETS - 1] / 1e9;
    float percent = 100.0f * static_cast<float>(mFrameBuckets[NUM_BUCKETS - 1]) / mTotalTime;
...
void SurfaceFlinger::dumpBufferingStats(String8& result) const {
    ...
    for (const auto& sortedPair : sorted) {
        ...
        result.appendFormat("  [%s] %.2f %.3f %.3f %.3f\n", std::get<0>(values).c_str(), activeTime,
                            std::get<1>(values), std::get<2>(values), std::get<3>(values));
    }
    result.append("\n");
}
...
void SurfaceFlinger::dumpAllLocked(const Vector<String16>& args, size_t& index,
    ...
    colorizer.reset(result);
    mCurrentState.traverseInZOrder([&](Layer* layer) { layer->dump(result, colorizer); });
    ...
    result.appendFormat("  last eglSwapBuffers() time: %f us\n"
                        ...
                        mTransactionFlags, 1e9 / activeConfig->getVsyncPeriod(),
                        activeConfig->getDpiX(), activeConfig->getDpiY(), !mGpuToCpuSupported);
    ...
    for (size_t d = 0; d < mDisplays.size(); d++) {
        ...
        mCurrentState.traverseInZOrder([&](Layer* layer) { layer->miniDump(result, hwcId); });
        result.append("\n");
    }
...
const Vector<sp<Layer>>& SurfaceFlinger::getLayerSortedByZForHwcDisplay(int id) {
...
bool SurfaceFlinger::startDdmConnection() {
    void* libddmconnection_dso = dlopen("libsurfaceflinger_ddmconnection.so", RTLD_NOW);
...
static status_t updateDimensionsLocked(const sp<const DisplayDevice>& displayDevice,
    ...
    if ((*requestedWidth > displayWidth) || (*requestedHeight > displayHeight)) {
        ALOGE("size mismatch (%d, %d) > (%d, %d)", *requestedWidth, *requestedHeight, displayWidth,
              displayHeight);
        return BAD_VALUE;
    }
...
status_t SurfaceFlinger::captureScreen(const sp<IBinder>& display,
    ...
    if (CC_UNLIKELY(producer == 0)) return BAD_VALUE;
...
void SurfaceFlinger::renderScreenImplLocked(const sp<const DisplayDevice>& hw, Rect sourceCrop,
                                            uint32_t reqWidth, uint32_t reqHeight,
                                            int32_t minLayerZ, int32_t maxLayerZ, bool yswap,
    ...
    if (mHwOrientation & DisplayState::eOrientationSwapMask) {
        filtering =
                static_cast<int32_t>(reqWidth) != hw_h || static_cast<int32_t>(reqHeight) != hw_w;
    } else {
        filtering =
                static_cast<int32_t>(reqWidth) != hw_w || static_cast<int32_t>(reqHeight) != hw_h;
    }
    ...
    if (hw->getDisplayType() == DisplayDevice::DISPLAY_PRIMARY &&
        ...
        rotation = static_cast<Transform::orientation_flags>((rotation_rot_90 ^ hw_rot_90) |
                                                             (rotation_flip_hv ^ hw_flip_hv));
    }

    // set-up our viewport
    engine.setViewportAndProjection(reqWidth, reqHeight, sourceCrop, hw_h, yswap, rotation);
...
status_t SurfaceFlinger::captureScreenImplLocked(const sp<const DisplayDevice>& hw,
    ...
    for (const auto& layer : mDrawingState.layersSortedByZ) {
        ...
        layer->traverseInZOrder(LayerVector::StateSet::Drawing, [&](Layer* layer) {
            secureLayerIsVisible =
                    secureLayerIsVisible || (layer->isVisible() && layer->isSecure());
        });
    }
    ...
    EGLImageKHR image =
            eglCreateImageKHR(mEGLDisplay, EGL_NO_CONTEXT, EGL_NATIVE_BUFFER_ANDROID, buffer, NULL);
    ...
    if (sync != EGL_NO_SYNC_KHR) {
        ...
        if (sync != EGL_NO_SYNC_KHR) {
            ...
            if (result == EGL_TIMEOUT_EXPIRED_KHR) {
                ...
                ALOGW_IF(eglErr != EGL_SUCCESS, "captureScreen: error waiting on EGL fence: %#x",
                         eglErr);
            }
            eglDestroySyncKHR(mEGLDisplay, sync);
        } else {
    ...
    *outSyncFd = syncFd;
    ...
    if (DEBUG_SCREENSHOTS) {
        ...
        checkScreenshot(reqWidth, reqHeight, reqWidth, pixels, hw, minLayerZ, maxLayerZ);
        delete[] pixels;
    }
...
void SurfaceFlinger::checkScreenshot(size_t w, size_t s, size_t h, void const* vaddr,
                                     const sp<const DisplayDevice>& hw, int32_t minLayerZ,
                                     int32_t maxLayerZ) {
    if (DEBUG_SCREENSHOTS) {
