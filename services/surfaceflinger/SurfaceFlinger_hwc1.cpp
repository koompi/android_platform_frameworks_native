...
void SurfaceFlinger::init() {
    ...
    mRenderEngine = RenderEngine::create(mEGLDisplay, mHwc->getVisualID(), 0);
    ...
    for (size_t i = 0; i < DisplayDevice::NUM_BUILTIN_DISPLAY_TYPES; i++) {
        ...
        if (mHwc->isConnected(i) || type == DisplayDevice::DISPLAY_PRIMARY) {
            ...
            sp<DisplayDevice> hw =
                    new DisplayDevice(this, type, hwcId, mHwc->getFormat(hwcId), isSecure, token,
                                      fbs, producer, mRenderEngine->getEGLConfig(), false);
...
int32_t SurfaceFlinger::allocateHwcDisplayId(DisplayDevice::DisplayType type) {
    return (uint32_t(type) < DisplayDevice::NUM_BUILTIN_DISPLAY_TYPES) ? type
...
status_t SurfaceFlinger::getDisplayConfigs(const sp<IBinder>& display,
    ...
    for (size_t c = 0; c < hwConfigs.size(); ++c) {
        ...
        info.presentationDeadline = hwConfig.refresh - sfVsyncPhaseOffsetNs + 1000000;
...
void SurfaceFlinger::resyncToHardwareVsync(bool makeAvailable) {
    ...
    const nsecs_t period = getHwComposer().getRefreshPeriod(HWC_DISPLAY_PRIMARY);
...
void SurfaceFlinger::onVSyncReceived(HWComposer* /*composer*/, int type, nsecs_t timestamp) {
...
void SurfaceFlinger::postComposition(nsecs_t refreshStartTime) {
    ...
    updateCompositorTiming(vsyncPhase, vsyncInterval, refreshStartTime, retireFenceTime);
    ...
    mDrawingState.traverseInZOrder([&](Layer* layer) {
        ...
        bool frameLatched = layer->onPostComposition(glCompositionDoneFenceTime, retireFenceTime,
                                                     compositorTiming);
        if (frameLatched) {
                        ...
                        if (state.surface != NULL) {
                            ...
                            if (mUseHwcVirtualDisplays &&
                                (SurfaceFlinger::maxVirtualDisplaySize == 0 ||
                                 (width <=
                                          static_cast<int>(SurfaceFlinger::maxVirtualDisplaySize) &&
                                  height <= static_cast<int>(
                                                    SurfaceFlinger::maxVirtualDisplaySize)))) {
...
void SurfaceFlinger::computeVisibleRegions(const sp<const DisplayDevice>& displayDevice,
    ...
    mDrawingState.traverseInReverseZOrder([&](Layer* layer) {
        ...
        if (layer->getLayerStack() != displayDevice->getLayerStack()) return;
    ...
    if (hasGlesComposition) {
        ...
        if (hw->getDisplayType() != DisplayDevice::DISPLAY_PRIMARY) {
            ...
            if (scissor != bounds) {
                ...
                engine.setScissor(scissor.left, height - scissor.bottom, scissor.getWidth(),
                                  scissor.getHeight());
            }
        }
    }
            ...
            if (!clip.isEmpty()) {
                switch (cur->getCompositionType()) {
                    ...
                    case HWC_OVERLAY: {
                        ...
                        if ((cur->getHints() & HWC_HINT_CLEAR_FB) && i && layer->isOpaque(state) &&
                            (state.alpha == 0xFF) && hasGlesComposition) {
...
uint32_t SurfaceFlinger::setClientStateLocked(const sp<Client>& client, const layer_state_t& s) {
    ...
    if (layer != 0) {
        ...
        if (what & layer_state_t::eAlphaChanged) {
            if (layer->setAlpha(uint8_t(255.0f * s.alpha + 0.5f))) flags |= eTraversalNeeded;
        }
...
void SurfaceFlinger::dumpAllLocked(const Vector<String16>& args, size_t& index,
    ...
    result.appendFormat("  last eglSwapBuffers() time: %f us\n"
                        ...
                        mTransactionFlags, 1e9 / hwc.getRefreshPeriod(HWC_DISPLAY_PRIMARY),
                        hwc.getDpiX(HWC_DISPLAY_PRIMARY), hwc.getDpiY(HWC_DISPLAY_PRIMARY),
                        !mGpuToCpuSupported);
...
status_t SurfaceFlinger::captureScreen(const sp<IBinder>& display,
    ...
    public:
        ...
        virtual bool handler() {
            ...
            result = flinger->captureScreenImplLocked(hw, producer, sourceCrop, reqWidth, reqHeight,
                                                      minLayerZ, maxLayerZ, useIdentityTransform,
                                                      rotation, isLocalScreenshot);
            static_cast<GraphicProducerWrapper*>(IInterface::asBinder(producer).get())
                    ->exit(result);
            return true;
        }
    };
    ...
    sp<MessageBase> msg =
            new MessageCaptureScreen(this, display, IGraphicBufferProducer::asInterface(wrapper),
...
status_t SurfaceFlinger::captureScreenImplLocked(const sp<const DisplayDevice>& hw,
                                                 ...
                                                 uint32_t reqHeight, int32_t minLayerZ,
                                                 int32_t maxLayerZ, bool useIdentityTransform,
                                                 Transform::orientation_flags rotation,
                                                 bool isLocalScreenshot) {
    ATRACE_CALL();
            ...
            if (result == NO_ERROR) {
                ...
                if (image != EGL_NO_IMAGE_KHR) {
                    ...
                    if (imageBond.getStatus() == NO_ERROR) {
                        ...
                        renderScreenImplLocked(hw, sourceCrop, reqWidth, reqHeight, minLayerZ,
                                               maxLayerZ, true, useIdentityTransform, rotation);

                        // Attempt to create a sync khr object that can produce a sync point. If
                        // that isn't available, create a non-dupable sync object in the fallback
                        // path and wait on it directly.
                        EGLSyncKHR sync;
                        ...
                        if (sync != EGL_NO_SYNC_KHR) {
                            ...
                            if (sync != EGL_NO_SYNC_KHR) {
                                EGLint result =
                                        eglClientWaitSyncKHR(mEGLDisplay, sync,
                                ...
                                if (result == EGL_TIMEOUT_EXPIRED_KHR) {
                                    ...
                                    ALOGW_IF(eglErr != EGL_SUCCESS,
                                             "captureScreen: error waiting on EGL fence: %#x",
                                             eglErr);
                                }
                                eglDestroySyncKHR(mEGLDisplay, sync);
                            } else {
                                ALOGW("captureScreen: error creating EGL fence: %#x",
                                      eglGetError());
                            }
                        }
                        if (DEBUG_SCREENSHOTS) {
                            ...
                            checkScreenshot(reqWidth, reqHeight, reqWidth, pixels, hw, minLayerZ,
                                            maxLayerZ);
                            delete[] pixels;
                        }
